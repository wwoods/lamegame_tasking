...
var Controls = UiBase.extend({
    init: function(graph) {
        ...
        self._graph = graph;
        self._statsController = stats.controller;
        ...
        for (var name in stats.controller.stats) {
            lb.add(name);
        }
...
var Graph = UiBase.extend({
    init: function(config) {
        ...
        this._display = $('<div class="display"></div>').appendTo(self);
        this._controls = new Controls(self).appendTo(self);
        this._overlay = $('<div class="overtop"></div>').appendTo(self);
    
        this._statsController = stats.controller;
        this._config = {
            stat: null,
            groups: [],
    ...
    update: function(configChanges) {
        ...
        $.ajax('getData', {
            ...
            error: function() { self._overlay.text("Failed to load data"); },
        });
    }
    ,
    ...
    _onLoaded: function(dataRaw, timeTo) {
        ...
        //For drawing graph, we explicitly don't use self._overlay, since that 
        //gets overwritten if we need are auto-updating as fast as we can.
...
var Dashboard = UiBase.extend({
    init: function() {
        var self = this;
        this._super('<div class="dashboard"></div>');
        var t = $('<table></table>').appendTo(this);
        this.empty()
        var r = $('<tr></tr>').appendTo(t);
        var s = ' style="position:relative;"';
        var w = $(window).width() * 0.5;
        var c = $('<td' + s + '></td>').appendTo(r);
        c.css('width', w + 'px');
...
var StatsApp = UiBase.extend({
    init: function() {
        var self = this;
        self._super('<div class="stats-app"></div>');
        stats.app = self;
        
        self._statsController = new StatController();
        stats.controller = self._statsController;
